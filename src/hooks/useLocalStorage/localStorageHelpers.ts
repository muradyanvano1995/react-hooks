...
const stringSerializer: UseLocalStorageSerializer<string> = {
  read: (raw) => raw,
  write: (value) => value,
}

const booleanSerializer: UseLocalStorageSerializer<boolean> = {
  read: (raw) => {
    if (raw === 'true') {
      return true
    }
...
const numberSerializer: UseLocalStorageSerializer<number> = {
  read: (raw) => {
    if (raw === 'NaN') {
      return Number.NaN
    }
    ...
    // String(-0) is "0"; negative zero is not preserved through storage.
...
const jsonSerializer: UseLocalStorageSerializer<unknown> = {
  read: (raw) => JSON.parse(raw) as unknown,
  write: (value) => JSON.stringify(value),
}

const dateSerializer: UseLocalStorageSerializer<Date> = {
  read: (raw) => {
...
const mapSerializer: UseLocalStorageSerializer<Map<unknown, unknown>> = {
  read: (raw) => {
...
const setSerializer: UseLocalStorageSerializer<Set<unknown>> = {
  read: (raw) => {
    const parsed: unknown = JSON.parse(raw)
    if (!Array.isArray(parsed)) {
...
export function createDefaultSerializer<T>(
  ...
  if (typeof defaultValue === 'string') {
    return stringSerializer as unknown as UseLocalStorageSerializer<T>
  }
  if (typeof defaultValue === 'boolean') {
    return booleanSerializer as unknown as UseLocalStorageSerializer<T>
  }
  if (typeof defaultValue === 'number') {
    return numberSerializer as unknown as UseLocalStorageSerializer<T>
  }
  if (defaultValue instanceof Date) {
    return dateSerializer as unknown as UseLocalStorageSerializer<T>
  }
  if (defaultValue instanceof Map) {
    return mapSerializer as unknown as UseLocalStorageSerializer<T>
  }
  if (defaultValue instanceof Set) {
    return setSerializer as unknown as UseLocalStorageSerializer<T>
  }
  // Objects, arrays, null, and other JSON-compatible defaults.
  return jsonSerializer as unknown as UseLocalStorageSerializer<T>
}
