...
- Direct-focus hooks (`useFocus`) return `{ focused, focus, blur }` from native target-bound `focus`/`blur` and `ownerDocument.activeElement`; do not treat descendant focus as direct focus and do not add focus traps or focus-within semantics.
